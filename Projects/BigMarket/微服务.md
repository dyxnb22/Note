...
> 当前版本：最终 7 个可部署微服务。market 内置策略与返利能力，message-job 负责 MQ 消费、终态补偿和积分奖二级 Outbox 派发，account 提供积分与额度 RPC（dev/local/test 本地实现，docker/secure 真实 Dubbo 远程实现）。截至 2026-07-18，审计报告中的 15 项问题已完成代码/DDL/文档修复；当前仍是有条件学习冻结，不等同于生产就绪。
...
- 审计报告中的 AUD-001～AUD-015 已处理：money-like 写路径保留原有业务幂等键，并补齐 durable ledger、CAS 状态转换、远程写补偿、终态监控和人工重放入口。
- Outbox 只有在 RabbitMQ publisher confirm 成功且未发生 return 时才进入完成态；DLQ 持久化使用显式 ack，数据库失败会 requeue，不会直接丢消息。
- 活动 SKU 与策略奖品库存都先建立 MySQL `reserved` 账本，再通过 Redis 原子预占；库存 Job 会合并 Redis pending set 与 durable `reserved` 账本扫描，队列/Redis 短暂丢失后仍可恢复。
- `UNKNOWN` 远程结果不立即释放库存或额度；会保留业务号并进入对账/延续任务。积分、配额、聊天扣费均按业务号查询终态，禁止盲目重复写入。
- 100301 已在默认 SQL 和平台配置中关闭；默认演示路径只保留已有履约实现的奖品。
- 当前工作树已通过 `mvn -q test`、MyBatis XML 解析、Shell 语法检查、`git diff --check` 和 Runtime Safety 99/99 校验；本次没有执行 Docker fresh volume、secure runtime 或动态 acceptance。
...
4.  活动 SKU 库存责任链校验（`ActivitySkuStockActionChain`，以业务号/预占账本保证恢复幂等）
...
-   库存：先写 durable `reserved` 账本，再由 Redis Lua 原子预占奖品库存；成功保留原奖，失败走兜底，后续由确认队列和库存 Job 完成 MySQL 投影
-   兜底：返回配置的积分/安慰奖
...
Infrastructure 里 `AwardDispatchSupport` 在同一本地事务中：

1.  写中奖记录
2.  写 task（`state=create`，消息体已序列化）
3.  标记抽奖单为已使用

事务提交后再发 MQ；只有 publisher confirm 成功且没有 publisher return 才标 `completed`。未确认、未路由或发送异常保持 `create/fail`，由 `SendMessageTaskJob` 扫描补偿。
...
-   `UNKNOWN` 远程结果进入业务号对账，不直接重复入账；达到重试上限的任务进入 `failed/manual_pending` 并由指标、告警和人工重放工具接管
-   其他奖品类型按对应 `IDistributeAward` 实现处理；默认演示主路径为积分奖，100301 已关闭以避免抽中未实现履约奖品
...
B. 库存高并发（Redis 预扣 + durable ledger + 异步投影）

| 层级   | 存储                                                         | 语义                                     |
| :----- | :----------------------------------------------------------- | :--------------------------------------- |
| 热路径 | Redis `strategy_award_count:{strategyId}_{awardId}` Lua 原子预占 | 抽奖实时                                 |
| 事实层 | `strategy_award_stock_decrement_ledger` / `activity_sku_stock_decrement_ledger` | `reserved → applied/released`，按业务预占键幂等 |
| 冷路径 | MySQL `stock_count_surplus` 批量 UPDATE                      | 库存 Job 从延迟队列和 durable reserved 账本恢复回写 |

Redis 负责高并发热路径，MySQL ledger 负责可恢复事实和一次性状态转换；队列只有在 MySQL 更新成功后 ACK。活动 SKU 兑换的拒绝结果使用 restore ledger，`UNKNOWN` 保留 `wait_pay`，不提前恢复库存。
...
1.  扣配额、建参与单
2.  策略引擎出奖
3.  写中奖记录与发奖 Outbox；策略库存预占确认失败时写 durable confirm task
4.  失败则按原订单周期补偿回退配额；远程 UNKNOWN 进入 central pending 对账，不盲目回滚或重复扣减
...
它不直接管积分账本——扣款/退款走 gateway → market-service → account-service；`chat_credit_session` 由 market 作为 authoritative session，chatbot 与 message-job 使用相同的 userId 分库规则；退款和 `deducting` 对账由 message-job-service 的 XXL-Job 扫描处理。默认 local provider 为免费 fallback，只有配置了可计费的 DeepSeek provider 才进入扣费链路。
...
sequenceDiagram
  ...
  alt 配置为可计费 provider
    CB->>GW: 扣积分(requestId, costPerAsk)
...
-   默认 `local` provider 的 cost 为 `0`；只有配置了非空 API key 的 `deepseek` provider 才默认进入收费模式
...
-   扣费：`/raffle/activity/chat_credit_deduct_by_token?amount={costPerAsk}&requestId=xxx`
...
-   account 成功但 market 会话确认失败时保持 `deducting`；`ChatDeductReconcileJob` 查询 `chat_{userId}_{requestId}` 的账户订单终态，只补本地状态，不重复扣款

在锁保护下，用一次数据库事务完成「改余额 + 记流水 + 登记异步通知（chat场景下没意义，积分兑换发货有用）」；减积分时余额不够就整笔失败；同一业务单号重复提交只认第一次，从而保证积分账准确、可追溯、可补偿（整体的积分变动动作）

**Step 5：调用 AI**

`provider` 由当前 Nacos 快照明确选择 `local` 或 `deepseek`，不因远程失败静默切换实现；local fallback 不产生真实扣费。可计费 AI 超时/异常进入退款流程，由 `chat_credit_session` + message-job 补偿任务按同一业务号幂等退款。
...
-   `answer`：AI 文本
-   `creditDeducted`：本次实际扣费金额（local 免费时为 0）
-   `creditBalance`：扣后余额
...
- `SendMessageTaskJob`、活动 SKU/策略奖品库存回写、Chat 退款、Chat `deducting` 对账和远程写对账任务均由本服务承载
- `RemoteWriteReconcileJob` 扫描 `pending_remote_write_task`，按最大重试次数推进失败状态；人工复核后可用 `scripts/replay-pending-remote-write.sh` 重放
- `BusinessMetricsPublisher` 暴露 remote failed、credit award failed、strategy stock pending/manual、chat refund/deducting 和 DLQ 指标；告警规则位于 `docs/dev-ops/prometheus/rules/big-market-alerts.yml`
- `replay-credit-award-task.sh`、`replay-strategy-stock-confirm.sh` 与 `replay-pending-remote-write.sh` 只按原业务号重放，money-like 任务默认人工审核
- RabbitMQ publisher confirm/return 失败不会把 Outbox 提前标记完成；DLQ 使用显式 ack，`mq_dead_letter` 落库失败会 requeue
...
- 额度 ledger 保存原始 `month/day`，跨日/月回滚仍恢复原 bucket；反向积分只接受合法金额，关闭账户和不存在账户不会继续交易
...
- 当前状态：**conditional learning freeze（有条件学习冻结）**。最终拓扑仍为 gateway/auth/admin/market/chatbot/message-job/account 七个可部署服务；不代表生产就绪。
- 本次工作树已通过：`mvn -q test`、MyBatis mapper XML 解析、Shell 语法检查、`git diff --check`、Runtime Safety 99/99。
- 积分奖闭环要求 `user_award_record`、`credit_award_task=dispatched` 与 account 积分流水同时成立；禁止绕过 Outbox 直写账户积分。
- 本次没有执行 Docker fresh volume、完整 secure overlay、动态 acceptance、真实 RabbitMQ/MySQL/Redis 故障注入或 Playwright；不能把静态测试结果写成运行态闭环验收。
- Docker smoke、fresh/secure runtime、动态配置和故障恢复需要在 Docker 环境中另行验证；当前文档只记录已验证和未验证边界，不将健康端点或静态 validator 单独视为业务完成证明。
- 任何历史 `quota_create` failed 记录都应先用 `scripts/replay-pending-remote-write.sh --dry-run` 审核，再按原业务号人工重放。
- 本文档同步依据：`docs/LEARNING-FREEZE.md`、`docs/data-and-outbox.md`、`docs/operations-checklist.md`；代码变更未创建 Git commit。
