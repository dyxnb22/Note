...
**口述示例：** BigMarket 是我用来系统练习 Java 后端、DDD 和分布式一致性的营销抽奖作品集，不把它包装成已经承载真实生产流量的系统。当前基于 Java 17、Spring Boot 3.5.x，按 Gateway、Auth、Admin、Market、Chatbot、Message-Job 和 Account 七个可部署服务拆分；领域内再按 Activity、Strategy、Award、Credit、Rebate 等业务语言建模。实时抽奖链路会先扣额度并创建唯一抽奖单，再从 Redis 预装配概率表选候选奖品，通过责任链和规则树完成抽前分流、抽后校验与库存预留。中奖事实和 Outbox task 同事务落库，MQ 至少一次投递，下游用稳定业务号、唯一约束、状态机和补偿任务实现一次业务效果。这个项目最值得讲的是跨 DB、MQ 和 RPC 的失败窗口如何收敛；局限是目前只有本地 Docker 学习拓扑和代码/演练证据，没有生产 HA、容量和灾备数据。

面试官继续追问时，选一次“抽奖单 → 候选奖 → 中奖记录 → task → MQ → account 流水”的链路画图，不要继续横向罗列中间件。
...
### Q43：请讲一个失败案例或故障演练，你如何定位并修复？

**答题要点：** 只讲自己实际复现过的 Case；若尚未执行，就明确称为“待验证方案”，不能把设计推演说成线上事故。一个高价值演练是 account RPC 在“远端已入账、响应返回前”超时：调用方先把结果标记为 `UNKNOWN`，保留原 awardOrderId，沿 award record、一级 task、credit_award_task 和 account 流水定位第一个未收敛节点；补偿任务先查询远端终态，确认未执行且可安全重试时才用原业务号重放。

修复不能只写“加重试”，还要补幂等唯一键、状态转移 CAS、超时/错误分类、退避上限、人工状态和 Trace/业务号关联。验收至少覆盖响应丢失、重复消息、Worker 在提交前后崩溃和查询接口失败，证明不会重复入账、提前释放库存或把未知结果报成失败。

### Q44：这个项目有哪些性能和可靠性指标？没有压测数字时怎么回答？

**答题要点：** 没测过就不报 QPS 提升百分比，先给指标定义和采集点。实时链路看抽奖成功率、P50/P95/P99、Redis/DB/RPC 分段耗时、线程池/连接池等待和错误分类；异步链路看 task backlog、最老任务年龄、重试/DLQ/manual_pending、端到端入账延迟和 `UNKNOWN` 收敛时间；一致性看重复业务号冲突、库存 `reserved/applied/released` 差异和对账不一致数。

压测时固定奖品/概率配置、数据规模、JDK/容器资源、并发模型和下游桩，分别测 Redis O(1) 表与 O(log n) 方案，并用故障注入验证性能优化没有破坏一次业务效果。最终报告同时给吞吐、尾延迟、错误率、资源和正确性；如果目前只有代码路径和复杂度分析，就明确把容量基线列为下一步，而不是声称“高并发已验证”。

### Q45：如果流量增长十倍，你先扩容还是先改架构？

**答题要点：** 先用指标找饱和点，不因“十倍”直接重构。Gateway/Market 等无状态实例可横向扩容，但要检查会话、限流和配置是否共享；抽奖热路径先看 Redis 热 key、Lua 耗时与库存分片，数据库看连接池、索引、锁等待和分片键，MQ 看分区/队列数、消费者并发、积压年龄和顺序约束。

扩容前必须确认幂等键和状态机在多实例下仍成立，扩容消费者也不能超过队列/分区提供的并行度。只有单库写入、热点 key 或跨分片查询成为经压测证明的瓶颈时，才引入库存分片、读写拆分或进一步拆服务；每一步都要有回滚、对账和容量复测。
