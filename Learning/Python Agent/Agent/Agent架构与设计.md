...
## 10. Harness 哲学：Harness = Model + Infrastructure

来自实际工程项目的核心认知，区分两件事：

```
Agency（智能行为）← 来自模型训练，不来自代码
Harness（运行环境）← 你写的代码，让模型能在特定领域操作
```

**你能做的只有 Harness，不是 Agency。** 所谓"构建 Agent"，准确表述是：
...
来自企业级安全 Agent 实践的核心不变量（SafeCodeAgent Enterprise）：
...
## 15. 卡死循环检测（Stuck-Loop Detection）
...
## 18. 执行前歧义检测（Pre-Task Clarification）
