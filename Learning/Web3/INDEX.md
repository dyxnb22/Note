...
| 阶段 | 主文档 |
|---|---|
| 系统边界 | [Web3 到底是什么](00_Foundations/Web3%20到底是什么：它和互联网、金融、数据库各是什么关系.md)、[身份/资产/状态/权限](00_Foundations/链上世界里的身份、资产、状态和权限.md)、[工程与机制两条线](00_Foundations/为什么要从工程和机制两条线同时学%20Web3.md) |
...
| 治理与合规 | [治理与合规速查](06_Governance_and_Crypto_Society/治理与合规速查.md) |
...
只有准备系统理解 Web3 时才按 `00 → 01 → 02 → 03 → 04 → 05` 推进；平时可从钱包、交易、合约、DeFi 或协议研究的实际问题直接进入。`06–09` 是治理、研究、构建和案例分支，不进入阅读进度。
...
选择系统路线时，完成标准是能画出一次交易从签名、广播、排序、执行到确认/失败的轨迹。只有开始研究具体协议时，才进一步记录状态、不变量、权限、攻击面和证据边界。
