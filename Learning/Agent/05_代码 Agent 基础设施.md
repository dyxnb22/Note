...
> **学习位置**：完成 `01 → 02 → 03 → 04` 后阅读。这是第一个 Coding Agent 专题；先掌握只读观察，再加入 Patch、写入和验证。
>
> **本章完成标准**：能说明“模型提出 patch → 应用预览/授权 → 写入 → 测试 → 根据失败继续”的每个边界，并知道哪些动作必须人工确认。

> **职责边界**：本文是 Coding Agent 的文件/仓库导航、Patch、验证、Git/Worktree 和修复闭环主文档。`03` 只讲 Agent 架构判断，`04` 只讲通用 Context，权限策略和沙箱治理见 [安全与可控性](./08_安全与可控性.md)。
...
## learn-claude-code 对照：Worktree 是任务隔离，不是 Git 装饰

s18 的核心是把 `task_id`、Agent 身份、分支和工作目录绑定成一个可追踪记录。创建 worktree 后，队友的 `bash/read/write` 都必须以绑定目录为 cwd；任务完成时再明确选择 keep 或 remove，并记录创建、切换、提交和清理事件。

这补充了本篇的 Git/Worktree 章节：隔离目录只能解决文件互相覆盖，不能自动解决权限、依赖安装、数据库副作用或错误合并。生产实现还需要路径校验、分支命名校验、清理失败恢复、并发锁和任务状态一致性。对应实验：[s18_worktree_isolation/code.py](./实践/learn-claude-code/s18_worktree_isolation/code.py)，综合行为见 [s20_comprehensive/code.py](./实践/learn-claude-code/s20_comprehensive/code.py)。
