...
| 11–12 | [MCP Server、Claude API](./agent-learning-projects/11_mcp_server/main.py) | [MCP](../../MCP与工具协议.md)、[LLM 调用基础](../../01_LLM调用基础.md) |
...
进入具体项目后，按它的 README 安装依赖；需要模型的项目先复制 `.env.example`，不要提交真实密钥。每完成一个项目，只记录：输入与入口、状态/工具轨迹、一次失败边界、你的修改及验证结果。
