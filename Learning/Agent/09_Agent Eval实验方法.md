...
> **学习位置**：在 `03–08` 后阅读；先定义 Agent 的任务、工具和副作用合同，再谈分数。本文不维护通用测试 Harness 的第二份实现。

> **本章完成标准**：能把一次任务拆成结果、轨迹、安全和成本四类可检查证据，并说明测试环境如何隔离真实副作用。
>
> **职责边界**：本文只评估完整 Agent 任务和轨迹；通用测试集、Mock Provider、Judge、Baseline 和 CI 见 [Eval 与测试体系](./10_Eval与测试体系.md)。
