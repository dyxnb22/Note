...
> **学习位置**：`12` 部署文章之后阅读；本文负责运行中怎么发现、处置和复盘事故，不重复发布对象和兼容性细节。

> **职责边界**：本文是 SLI/SLO、告警、灰度 runbook、降级、Kill Switch、事故分级和复盘的主文档。发布对象、配置版本、Checkpoint 兼容性和 Worker 生命周期的部署细节见 [Agent 部署与生产化](12_部署与生产化.md)。
...
项目准备长期运行或对外提供服务时，再基于真实部署填写以下内容；概念学习和一次性本地实验不要求先写完整运维方案：
