...
> **学习位置**：这是成本/性能和模型行为的可选专题。先完成标准模型 + 工具 + Eval 基线，再评估额外推理预算是否值得。

> **职责边界**：本文只讨论额外推理预算、工具连续性、摘要/签名状态和评测取舍；不把可见 Thinking 当作原始思维链，也不替代安全、成功证据或 Trace。
...
只有任务基线表明失败主要来自多步推理时，才选一组包含简单、困难和工具型任务的数据，对普通、低/中/高推理投入分别记录通过率、首 token、总耗时、推理 token、工具次数和成本。目标是得到可执行路由规则，不是为了完成文档练习而固定做满 20 条。
