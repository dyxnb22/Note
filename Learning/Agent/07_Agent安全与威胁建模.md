...
> **学习位置**：完成 `02 Tool Calling`、`03 Agent 架构` 和 `05 代码 Agent` 后阅读。安全不是单独给 Prompt 加一段规则，而是从信任边界倒推工具、权限和执行隔离。
>
> **本章完成标准**：能画出一个任务的信任边界，并把每个威胁绑定到具体防线和失败/审计证据，而不是只说“加强 Prompt”。
