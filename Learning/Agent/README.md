...
新入口先看[00_学习路线图](./00_学习路线图.md)，它负责阶段、练习和验收；本页只负责文档地图。
...
本目录按“一个主题一份主文档”维护。其他文档可以提到同一机制，但只负责说明它与本主题的关系，不再复制完整实现。

- 想理解概念和取舍，读主文档；想看可运行代码，进入 `实践/`。
- 同一问题有多个入口时，优先看下面的职责边界表，再沿链接深入。
- 代码块只作为合同、伪代码或局部示例；涉及状态、权限、预算、副作用或恢复时，必须在附近说明边界和失败路径。Provider、SDK 和价格以 [版本与来源](./版本与来源.md) 为准。
- 发现两个文档都在维护同一段规则或实现时，保留更靠近运行时边界的一份，另一份改为链接和差异说明。
...
知识 Agent：文档摄取 → 检索/RAG → 权限过滤 → Context/Citation → Memory
...
| 组别 | 文档 |
|---|---|
| 核心 | [01_LLM调用基础](./01_LLM调用基础.md)、[02_Tool Calling](./02_Tool%20Calling.md)、[03_Agent架构与设计](./03_Agent架构与设计.md)、[04_Context工程](./04_Context工程.md)、[05_代码 Agent 基础设施](./05_代码%20Agent%20基础设施.md) |
| 产品与选型 | [Agent 产品与人机协同](./Agent产品与人机协同.md)、[Agent框架与平台选型](./Agent框架与平台选型.md) |
...
| 输出与面试 | [学习范围与面试题型](./面试/学习范围与面试题型.md)、[AI 项目表达与面试](./项目表达与面试.md)、[Career 通用项目表达](../Career/项目表达.md) |
| Rust 实现 | [Rust Agent 工程化](../Backend/Rust/16_Rust%20Agent工程化.md)、[rust-agent-runtime](./实践/rust-agent-runtime/README.md) |

## 交叉主题的职责边界

这张表是去重规则，不只是导航：主文档负责完整解释和实现，相关文档只保留自己的边界与交叉影响。
...
| 运行时 | [03_Agent架构与设计](./03_Agent架构与设计.md) | Agent Loop、状态和运行时机制；[02_Tool Calling](./02_Tool%20Calling.md) 负责工具合同、执行与路由，[Workflow 与编排](./Workflow与编排.md) 负责通用编排，[LangGraph](./LangGraph.md) 负责框架实现，[Agent框架与平台选型](./Agent框架与平台选型.md) 负责生态对比 |
| RAG / 知识 | [RAG](./RAG.md) | 应用链路、Context 装配、Grounding、生成和端到端评测；[知识系统](./知识系统.md) 负责来源、版本、权限、引用和删除治理；[文档摄取与解析](./文档摄取与解析.md) 负责 OCR/版面/表格与质量门禁；[检索系统工程](./检索系统工程.md) 负责检索基础设施；[GraphRAG与关系检索](./GraphRAG与关系检索.md) 负责多跳实体关系 |
...
- 知识型 Agent：`LLM 调用 → Context → 文档摄取与解析 → RAG → 检索系统工程 → 知识系统 → Citation → Eval`。
