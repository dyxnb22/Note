...
| 运行时 | [Agent 架构与设计](./Agent架构与设计.md) | Agent Loop、状态和运行时机制；[Tool Calling](./Tool%20Calling.md) 负责工具合同/执行，[Workflow 与编排](./Workflow与编排.md) 负责通用编排，[LangGraph](./LangGraph.md) 负责框架实现 |
...
| 部署 | [部署与生产化](./部署与生产化.md) | 只负责模型/Prompt/工具/索引版本、Eval 门禁、状态恢复和 LLM 预算；通用 Docker、CI/CD、健康检查和发布流程见 [Backend Delivery](../Backend/Delivery/README.md) |
