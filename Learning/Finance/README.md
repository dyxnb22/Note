# Learning/Finance 知识库总览

## 关于本知识库

本知识库是一套系统化的个人金融与经济学学习文档，目标是建立从底层概念到实际研究方法的完整知识体系。

写作原则：机制优先、框架优先、长期有效优先。不荐股、不喊单、不依赖实时数据。

---

## 目录结构

- [00_Foundations](/Users/diaoyuxuan/Documents/Notes/Learning/Finance/00_Foundations/README.md)：基础层，建立时间价值、风险收益、概率思维
- [01_Economics](/Users/diaoyuxuan/Documents/Notes/Learning/Finance/01_Economics/README.md)：经济学机制、政策工具、经济周期
- [02_Markets](/Users/diaoyuxuan/Documents/Notes/Learning/Finance/02_Markets/README.md)：金融体系、流动性、利率期限结构
- [03_Stocks](/Users/diaoyuxuan/Documents/Notes/Learning/Finance/03_Stocks/README.md)：股票机制、财报分析、商业模式
- [04_Fixed_Income](/Users/diaoyuxuan/Documents/Notes/Learning/Finance/04_Fixed_Income/README.md)：债券定价、利率风险、信用风险
- [05_Funds_and_ETF](/Users/diaoyuxuan/Documents/Notes/Learning/Finance/05_Funds_and_ETF/README.md)：基金与 ETF 的结构、评价与定投
- [06_Derivatives](/Users/diaoyuxuan/Documents/Notes/Learning/Finance/06_Derivatives/README.md)：期货、期权、对冲、杠杆
- [07_Corporate_Finance](/Users/diaoyuxuan/Documents/Notes/Learning/Finance/07_Corporate_Finance/README.md)：资本结构、投融资决策、代理问题
- [08_Valuation](/Users/diaoyuxuan/Documents/Notes/Learning/Finance/08_Valuation/README.md)：DCF、相对估值、安全边际
- [09_Portfolio_and_Risk](/Users/diaoyuxuan/Documents/Notes/Learning/Finance/09_Portfolio_and_Risk/README.md)：组合构建、风险控制、再平衡
- [10_Behavioral_Finance](/Users/diaoyuxuan/Documents/Notes/Learning/Finance/10_Behavioral_Finance/README.md)：认知偏差、情绪管理、纪律建设
- [11_Quant](/Users/diaoyuxuan/Documents/Notes/Learning/Finance/11_Quant/README.md)：因子、回测、容量、执行归因
- [12_Math_and_Statistics](/Users/diaoyuxuan/Documents/Notes/Learning/Finance/12_Math_and_Statistics/README.md)：收益率、分布、回归、统计检验
- [13_Research_Workflow](/Users/diaoyuxuan/Documents/Notes/Learning/Finance/13_Research_Workflow/README.md)：读公司、写备忘录、复盘与研究流程
- [14_Fraud_and_Pitfalls](/Users/diaoyuxuan/Documents/Notes/Learning/Finance/14_Fraud_and_Pitfalls/README.md)：骗局识别、造假识别、风险陷阱

---

## 推荐学习路径

**第一阶段：建立底层语言**
`00_Foundations` → `12_Math_and_Statistics` → `01_Economics`（核心篇）

**第二阶段：理解市场与资产**
`02_Markets` → `03_Stocks`（基础篇）→ `04_Fixed_Income`（基础篇）→ `07_Corporate_Finance`

**第三阶段：深入资产分析**
`03_Stocks`（全部）→ `04_Fixed_Income`（全部）→ `08_Valuation` → `05_Funds_and_ETF`

**第四阶段：组合、衍生与风险**
`09_Portfolio_and_Risk` → `06_Derivatives` → `10_Behavioral_Finance`

**第五阶段：量化与研究方法**
`11_Quant` → `13_Research_Workflow`

**防御层（全程穿插）**
`14_Fraud_and_Pitfalls`

---

## 关键先修关系

- 所有模块的前提：`00_Foundations`
- 估值的前提：`03_Stocks` + `07_Corporate_Finance`
- 量化的前提：`12_Math_and_Statistics` + `09_Portfolio_and_Risk`
- 研究方法的前提：`08_Valuation` + `10_Behavioral_Finance`

---

## 写作规范说明

本知识库所有文档遵守统一模板，章节顺序为：
主题定位 → 核心概念 → 核心机制 → 分析框架 → 关键指标 → 实际例子 → 常见误区 → 主题关系 → 练习 → 一页总结 → 术语表

每篇文档末尾以一行反引号标签结尾，格式为：`` `#tag1 #tag2 #tag3` ``

涉及监管、税制、费率、交易制度的内容，统一标注：
> 此部分可能随监管、交易所规则、产品设计变化，需以最新规则为准。

---

## 文档状态追踪

| 模块 | 文档（篇数）| 待补充 |
|------|-----------|--------|
| 00_Foundations | ✅ 时间价值与贴现、复利与对数收益率、风险与收益的基本关系、机会成本与边际思维、概率直觉与统计基础、个人资产负债表与现金流、保险、负债与风险转移（7 篇）| — |
| 01_Economics | ✅ 经济学十大核心概念、供给与需求、通货膨胀、利率机制、汇率机制、货币与信用、货币政策、财政政策、经济周期、失业与劳动力市场、市场效率与市场失灵、信用周期与资产价格（12 篇）| — |
| 02_Markets | ✅ 金融体系总览、金融中介与金融市场、市场结构与流动性、利率期限结构、中国市场与海外市场差异概览（5 篇）| — |
| 03_Stocks | ✅ 股票是什么、股东权益与公司价值、财报分析、关键财务指标、基本面分析框架、护城河与商业模式、指数行业与风格、A股港股美股差异、股票投资常见误区（9 篇）| — |
| 04_Fixed_Income | ✅ 债券基础、到期收益率、久期与凸性、利率风险、信用风险、国债信用债可转债、固收在资产配置中的作用（7 篇）| — |
| 05_Funds_and_ETF | ✅ 基金类型总览、主动与被动投资、跟踪误差、基金评价指标、定投的逻辑与误区（5 篇）| — |
| 06_Derivatives | ✅ 期货基础、期权基础、常见期权策略原理、杠杆与保证金、对冲与投机、衍生品风险（6 篇）| — |
| 07_Corporate_Finance | ✅ 资本结构、WACC、投资决策与资本预算、分红回购与再融资、管理层激励与代理问题（5 篇）| — |
| 08_Valuation | ✅ DCF估值、PE/PB/PS/EV/EBITDA、相对估值框架、安全边际、估值为什么会失效（5 篇）| — |
| 09_Portfolio_and_Risk | ✅ 分散化与相关性、波动率与最大回撤、夏普比率与绩效指标、仓位管理、风险预算、再平衡、黄金、商品与另类资产（7 篇）| — |
| 10_Behavioral_Finance | ✅ 损失厌恶、过度自信、锚定效应、从众效应、叙事偏差、情绪管理与投资纪律（6 篇）| — |
| 11_Quant | ✅ 量化投资是什么、因子投资框架、Alpha与Beta、回测原则、回测偏差、滑点交易成本与容量、信号组合执行归因、数据清洗与预处理（8 篇）| — |
| 12_Math_and_Statistics | ✅ 收益率的计算与转换、期望方差与协方差、相关性与回归分析、常用概率分布、统计检验基础、蒙特卡洛模拟（6 篇）| — |
| 13_Research_Workflow | ✅ 如何读一家公司、投资备忘录写作、信息源与研究工作流、投资复盘方法、行业研究方法、观察清单管理、研究观点与交易决策的区分、如何避免信息过载（8 篇）| — |
| 14_Fraud_and_Pitfalls | ✅ 财务造假识别、杠杆爆仓机制、伪量化识别、常见投资骗局、流动性陷阱与概念炒作、幸存者叙事识别（6 篇）| — |

---

`#finance #index #knowledge-base`
