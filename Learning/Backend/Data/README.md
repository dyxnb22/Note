...
1. 先看 `数据库系统原理`，建立数据模型、索引、事务和恢复框架
2. 再看 `SQL`，掌握关系查询语言
3. 然后用 `MySQL` 或 `PostgreSQL` 对照具体实现
4. 再看 `Redis`
5. 最后看 `消息队列`
6. 需要分析与实时数据链路时进入 `数据工程与流处理`
...
- 数据平台：`大数据基础` 用于组件地图，`数据工程与流处理` 负责 CDC、批流、仓库/湖仓、编排和治理主线。
- `海量数据处理` 负责算法型场景；向量索引和混合检索以 Agent 下的 `检索系统工程` 为唯一主文档。
