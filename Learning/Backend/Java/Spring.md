...
## Spring MVC 常见参数绑定与接口约定

Controller 方法的参数绑定主要看请求数据所在位置：

| 请求数据 | 常用写法 | 说明 |
| --- | --- | --- |
| 查询参数 | `@RequestParam String name` | 参数名不一致时显式指定 `name`；可设置 `required` 和 `defaultValue` |
| 表单或查询参数对象 | `UserQuery query` | 参数名与对象属性名匹配，由 Spring 完成数据绑定 |
| 数组/集合 | `@RequestParam List<Long> ids` | 同名参数可绑定为集合，接口应明确是否允许空集合 |
| 日期时间 | `@DateTimeFormat(...) LocalDateTime time` | 明确输入格式，避免依赖环境默认格式 |
| JSON 请求体 | `@RequestBody CreateCommand command` | 由 `HttpMessageConverter` 反序列化，请求头通常为 `application/json` |
| 路径变量 | `@PathVariable Long id` | 资源标识放在 URL 路径中 |

RESTful URL 是约定，不是框架强制规则。通常用名词描述资源，用 HTTP 方法表达动作，并统一响应结构，例如 `code`、`message` 和 `data`。统一响应不代表所有错误都返回 HTTP 200；鉴权失败、参数错误和服务端异常仍应使用有意义的 HTTP 状态码。
...
## Spring Boot Web 的文件、配置与异常处理

### 文件上传

文件上传通常使用 `MultipartFile`。服务端至少需要：

- 校验大小、扩展名、MIME 类型和文件内容，不能只相信客户端传来的文件名。
- 使用 UUID 或业务 ID 生成服务端文件名，避免覆盖和路径穿越；原始文件名只作为展示信息保存。
- 本地磁盘适合开发或单机临时场景，生产环境通常使用对象存储，并在数据库保存对象 key 或访问地址。
- 下载和访问也要做权限校验，不能因为文件上传成功就默认公开。

### 外部配置

- `@Value` 适合注入少量配置项。
- `@ConfigurationProperties` 适合把同一前缀下的配置绑定为类型安全的配置对象，并集中做校验。
- YAML、properties 都只是配置表达形式；密钥应由环境变量、密钥管理系统或部署平台注入，不应提交到仓库。

### 全局异常处理

Controller 中逐个 `try/catch` 会让接口代码混入重复的错误处理。更常见的做法是使用 `@RestControllerAdvice` 配合 `@ExceptionHandler`：

1. 业务异常映射为稳定的错误码和客户端可理解的消息。
2. 参数校验异常统一转换为字段级错误。
3. 未预期异常记录完整服务端日志，但只向客户端返回通用错误信息，避免泄露堆栈、SQL 和敏感配置。
...
## MyBatis 动态 SQL 的常用写法

动态 SQL 的目标是根据参数安全地拼接条件，而不是把用户输入直接拼进 SQL：
...
- `<where>` 在有条件时自动添加 `WHERE`，并清理开头多余的 `AND`/`OR`。
- `<set>` 适合动态更新，自动清理末尾逗号。
- `<foreach>` 适合安全地展开集合参数，例如 `IN (...)`；集合为空时要在业务层提前处理。
- `<sql>` + `<include>` 可以复用字段列表，但不要把复杂业务逻辑隐藏在大量 XML 片段中。
...
## MyBatis 延迟加载和插件有什么注意点？

延迟加载是在真正访问关联对象时才查询，能减少不必要的关联查询，但容易产生 N+1 查询；接口返回前要确认会话仍然有效，必要时使用明确的批量查询或一次性 JOIN。

MyBatis 插件通过拦截 Executor、StatementHandler、ParameterHandler 或 ResultSetHandler 等扩展点，在 SQL 执行前后增加分页、审计、性能统计等逻辑。插件要控制拦截范围，避免重复改写 SQL、影响事务或引入难以排查的性能问题。
